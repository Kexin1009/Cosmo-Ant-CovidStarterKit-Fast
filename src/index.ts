...
edges.then((data) =>{
  for (let node = 0; node < data.vertices.length; node += 1) {
    ...
    nodes.push( {id: `${data.vertices[node].v_id}`} );
  }
  // for (let node = 0; node < data.entities.length; node += 1) {
  //   // console.log(data.vertices[node].attributes.content);
  //   nodes.push({ id: `${data.entities[node].v_id}`, content: data.entities[node].v_id });
  // }

  for (let node = 0; node < data.edges.length; node += 1) {
    links.push({ source: `${data.edges[node].from_id}`, target: `${data.edges[node].to_id}`});
    // links.push({ source: `${data.edges[node].from_id}`, target: `${data.edges[node].to_id}`, colour: `white`,  directed: data.edges[node].directed});
